...
USING (
  EXISTS (
    SELECT 1 
    FROM get_current_user_data() AS current_user
    WHERE 
      -- Admin, supervisor, rh podem ver todas as metas
      current_user.tipo IN ('admin', 'supervisor', 'rh')
      OR
      -- Gerentes e líderes podem ver metas da sua loja
      (current_user.tipo IN ('gerente', 'lider', 'sublider', 'subgerente', 'auxiliar', 'farmaceutico', 'consultora') 
       AND EXISTS (
         SELECT 1 FROM usuarios u 
         WHERE u.id = metas.usuario_id 
         AND u.loja_id = current_user.loja_id
       ))
      OR
      -- Usuários podem ver suas próprias metas
      metas.usuario_id = current_user.id
  )
);
...
WITH CHECK (
  EXISTS (
    ...
    WHERE 
      ...
      (current_user.tipo IN ('gerente', 'lider', 'sublider', 'subgerente') 
       AND EXISTS (
         SELECT 1 FROM usuarios u 
         WHERE u.id = metas.usuario_id 
         AND u.loja_id = current_user.loja_id
...
USING (
  EXISTS (
    ...
    WHERE 
      ...
      -- Gerentes e líderes podem ver metas da sua loja
      (current_user.tipo IN ('gerente', 'lider', 'sublider', 'subgerente') 
       AND metas_loja.loja_id = current_user.loja_id)
  )
);
...
WITH CHECK (
  EXISTS (
    SELECT 1 
    FROM get_current_user_data() AS current_user
    WHERE current_user.tipo IN ('admin', 'supervisor', 'rh')
  )
);
...
USING (
  EXISTS (
    ...
    WHERE 
      ...
      (current_user.tipo IN ('gerente', 'lider', 'sublider', 'subgerente') 
       AND ml.loja_id = current_user.loja_id)
  )
);

-- Política para inserção de categorias de metas de loja (apenas admin/supervisor/rh)
CREATE POLICY "Inserir categorias de metas - admin apenas"
ON metas_loja_categorias 
FOR INSERT 
WITH CHECK (
  EXISTS (
...
-- Política para atualização de categorias de metas de loja (apenas admin/supervisor/rh)
CREATE POLICY "Atualizar categorias de metas - admin apenas"
ON metas_loja_categorias 
FOR UPDATE 
USING (
  EXISTS (
