...
export interface VendasConsolidadas {
  ...
  // Novos campos para categorias específicas
  rentaveis: number;
  perfumaria_alta: number;
  conveniencia_alta: number;
  goodlife: number;
}
...
// Mapeamento dos grupos por categoria conforme especificado
const GRUPOS_POR_CATEGORIA = {
  'rentaveis': [20, 25], // R+ Rentáveis
  'perfumaria_alta': [46], // Perfumaria Alta Rentabilidade  
  'conveniencia_alta': [36, 13], // Conveniência Alta Rentabilidade
  'goodlife': [22], // GoodLife/Saúde
  'geral': [] // Sem filtro - todos os grupos
};
...
export const useCallfarmaAPI = () => {
  ...
  const buscarVendasConsolidasCallfarma = async (
    ...
    try {
      ...
      // Inicializar categorias específicas
      let rentaveis = 0;
      let perfumariaAlta = 0;
      let convenienciaAlta = 0;
      let goodlife = 0;
      ...
      dadosDetalhados.forEach((item: any) => {
        ...
        if (valorLiquido > 0) {
          ...
          // Somar por categorias específicas conforme mapeamento
          if (GRUPOS_POR_CATEGORIA.rentaveis.includes(grupoId)) {
            rentaveis += valorLiquido;
          } else if (GRUPOS_POR_CATEGORIA.perfumaria_alta.includes(grupoId)) {
            perfumariaAlta += valorLiquido;
          } else if (GRUPOS_POR_CATEGORIA.conveniencia_alta.includes(grupoId)) {
            convenienciaAlta += valorLiquido;
          } else if (GRUPOS_POR_CATEGORIA.goodlife.includes(grupoId)) {
            goodlife += valorLiquido;
          }
        }
      });
      ...
      const resultado: VendasConsolidadas = {
        ...
        rentaveis,
        perfumaria_alta: perfumariaAlta,
        conveniencia_alta: convenienciaAlta,
        goodlife
      };
      ...
      console.log('✅ Vendas consolidadas da API Callfarma:', {
        ...
        'Rentáveis': `R$ ${rentaveis.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`,
        'Perfumaria Alta': `R$ ${perfumariaAlta.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`,
        'Conveniência Alta': `R$ ${convenienciaAlta.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`,
        'GoodLife': `R$ ${goodlife.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`,
  ...
  const buscarVendasCompletasComGrupos = async (
    ...
    try {
      ...
      if (filtroLoja) {
        paramsGraficos.filtroFiliais = filtroLoja;
      }
  ...
  // Função original para buscar vendas de funcionários (mantida para compatibilidade)
